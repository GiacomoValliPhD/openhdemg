...
class ErrorDialog:
    ...
    This class creates a custom dialog window using customtkinter (ctk) components to show an error message
    and a potential solution. The dialog includes an information icon, labels for the error and solution,
    and is styled with specific foreground and background colors.

    Parameters
    ----------
    parent : tk.Tk or ctk.CTk
        The parent window to which this dialog is attached. It can be either a Tkinter root window
        or another customtkinter component.
    error : str
        The error message to be displayed in the dialog. This should describe what went wrong.
    solution : str
        A message providing a potential solution or workaround for the error described.
    ...
    content_frame : ctk.CTkFrame
        A frame widget that holds the content of the dialog, including the error and solution messages.
    ...
    __init__(self, parent, error, solution)
        Initializes the ErrorDialog with the parent window, error message, and solution message.
    ...
    def __init__(self, parent, error, solution):
        ...
        self.content_frame = ctk.CTkFrame(self.head, corner_radius=10, fg_color="LightBlue4", bg_color="#FFBF00")
        self.content_frame.pack(padx=50, expand=True, fill="both")

        # Load an information icon and display it
        self.info_photo = ctk.CTkImage(light_image=Image.open(path + "/Error.png"), size=(50, 50))
        self.icon = ctk.CTkLabel(self.content_frame, text="",
                                 image=self.info_photo, bg_color="LightBlue4")
        self.icon.pack(pady=5)
        self.icon_info = ctk.CTkLabel(self.content_frame,
                                      text="INFORMATION", font=("Arial", 16, "bold"),
                                      text_color="#123456")
        self.icon_info.pack(pady=5)

        # Error solution label (larger, bold), placed below the icon
        self.solution_label = ctk.CTkLabel(self.content_frame, text=solution,
                                           font=("Arial", 14, "bold"), wraplength=400, fg_color="LightBlue4")
        self.solution_label.pack(pady=(10, 5))

        # Error traceback label (smaller)
        self.error_label = ctk.CTkLabel(self.content_frame, text=error,
                                        font=("Arial", 10), wraplength=400, fg_color="LightBlue4")
...
def show_error_dialog(parent, error, solution):
    """
    Displays an ErrorDialog with a formatted error traceback and a solution message.

    This function takes an exception as input, formats its traceback, and then creates
    an ErrorDialog to display the formatted traceback along with a solution message.
